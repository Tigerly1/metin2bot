...
class BossFarmBot_50lvl:
    ...
    def run(self):
        while not self.stopped:
            ...
            if self.state == BotState.INITIALIZING:
                ...
                #self.turn_on_buffs()
                self.switch_state(BotState.KILLING_BOSSES)
            ...
            if self.state == BotState.HITTING:
                ...
                result = self.get_mob_info()
                #print(result)
                if result is None or time.time() - self.started_hitting_time >= 30:
                    self.started_hitting_time = None
            ...
            if self.state == BotState.KILLING_BOSSES:
                ...
                self.osk_window.start_spinning()
                time.sleep(0.03)
                ...
                if self.boss_count % 50 == 49:
                     self.turn_on_polymorphy()
    ...
    def teleport_to_next_boss(self, respawn_number=1):
       
        metin_tp_page = [(586,587),(475,606)]
        ...
        time.sleep(0.1)
        self.metin_window.mouse_move(metin_tp_page[math.floor((respawn_number-1)/6)][0], metin_tp_page[math.floor((respawn_number-1)/6)][1])
        time.sleep(0.04)
        self.metin_window.mouse_click()

        time.sleep(0.1)
        self.metin_window.mouse_move(coords[respawn_number%6-1][0], coords[respawn_number%6-1][1])
        time.sleep(0.04)
        self.metin_window.mouse_click()
        time.sleep(8)
    ...
    def change_metin_respawn_or_channel(self):
        ...
        if self.current_metin_respawn == 1 and self.current_channel % 2 == 0 \
                    and self.metin_teleports_passed % 11 == 0 and self.metin_teleports_passed > 0:
            self.current_channel = (self.current_channel % 8) + 1
            self.change_channel(self.current_channel)
        elif self.current_metin_respawn == 12 and self.current_channel % 2 == 1 \
              and self.metin_teleports_passed % 11 == 0:
            self.current_channel = (self.current_channel % 8) + 1
            self.change_channel(self.current_channel)
        else:
            if self.metin_teleports_passed % 22 >= 11:
                self.current_metin_respawn = self.current_metin_respawn - 1
                self.metin_teleports_passed += 1
                self.teleport_to_next_boss(self.current_metin_respawn)
            else:
                self.current_metin_respawn = (self.current_metin_respawn % 12) + 1
                self.metin_teleports_passed += 1
                self.teleport_to_next_boss(self.current_metin_respawn)
